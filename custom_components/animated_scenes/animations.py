...
COLOR_GROUP_SCHEMA = {
	vol.Optional(ATTR_BRIGHTNESS): vol.Any(
...
class Animation:
	def __init__(self, hass, config):
		self._unique_id: str = config.get(CONF_UNIQUE_ID)
	...
	def find_nearby_color(self, color):
		selected_color = [color[CONF_COLOR][0], color[CONF_COLOR][1], color[CONF_COLOR][2]]
		modifier = color[CONF_NEARBY_COLORS]
		if color[CONF_COLOR_TYPE] not in [ATTR_RGB_COLOR, ATTR_RGBW_COLOR, ATTR_RGBWW_COLOR]:
		...
		r, g, b = map(lambda x: 255 if x > 255 else 0 if x < 0 else int(x), colorsys.hls_to_rgb(hmod, lmod, smod))
		return [r,g,b]
...
class Animations:
	...
	def release_animation(self, animation: Animation):
		del self.animations[animation._unique_id]
	...
	async def release_light(self, animation: Animation, entity_id):
		...
		if self._light_owner[entity_id] != animation:
			return _LOGGER.info(
				...
				entity_id,
				self._light_owner[entity_id]._unique_id,
			)
